...
export function MainLayout() {
  ...
  const onMouseDown = useCallback((e: React.MouseEvent) => {
    if (!sidebarConfig.resizable) return
    e.preventDefault()
    dragging.current = true
    startX.current = e.clientX
    startW.current = boundedSidebarWidth

    const onMouseMove = (ev: MouseEvent) => {
      if (!dragging.current) return
      const delta = ev.clientX - startX.current
      const next = clamp(startW.current + delta, sidebarConfig.min_width, sidebarConfig.max_width)
  ...
  }, [boundedSidebarWidth, sidebarConfig.max_width, sidebarConfig.min_width, sidebarConfig.resizable])
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div
          ...
          style={{ width: collapsed ? 0 : boundedSidebarWidth, transition: collapsed ? 'width 0.15s ease' : undefined }}
        >
          <div
            className="h-full overflow-hidden"
            style={{ width: collapsed ? 0 : boundedSidebarWidth, minWidth: collapsed ? 0 : boundedSidebarWidth }}
          >
            <Sidebar />
          </div>

          {/* Drag handle */}
          {!collapsed && sidebarConfig.resizable && (
            <div
