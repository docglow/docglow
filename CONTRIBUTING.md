...
### Verify your branch before submitting

Passing tests tells you the code you wrote does what you expected. It does not
tell you whether the change holds up under flags you did not use, against dbt
artifact shapes you did not think about, or at project sizes larger than the
example. Most review findings on this repo come from that second category.

The repo ships a verification skill that covers both. If you use Claude Code,
run:
...
Otherwise read [`.claude/skills/pr-verify/SKILL.md`](.claude/skills/pr-verify/SKILL.md)
and work through it by hand — it is a plain checklist with the commands written
out, and does not require any particular tooling.

The two phases are:

1. **Conformance** — the full Python and frontend suites, lint, type checks,
   and a rebuild-and-compare of the vendored frontend bundle if you touched
   `src/docglow/static/`.
2. **Behavioral** — generating real sites under `--select`, `--slim`, and with
   `run_results.json` absent, then reading the emitted payload; spot-checking
   artifact joins against the raw manifest for every category of input; walking
   the pipeline stage list for contract violations; and measuring the size of
   any new payload key.

Doing this yourself makes review much faster, and it is the same process a
maintainer will run on your PR.

Two conventions the skill checks that are easy to miss:

- New top-level keys on `DocglowData` are optional (`readonly key?: T`) in
  `packages/shared-types/src/site.ts`, and the mirror in
  `frontend/src/types/index.ts` must declare the same optionality. Sites
  generated by older versions do not have your key.
- If you add a top-level payload key, update the exact-key assertions in
  `tests/test_data_transformer.py` and `tests/test_dbt_versions.py`.
